function EventPage({ locationId }) {
    ...
    return (
      <div className="flex flex-col h-screen bg-[#fef5f1]">
        ...
        <button
          ...
          className="bg-[#d3b4a4] text-white py-2 rounded-lg font-semibold"
        >
          + Create Your Table
        </button>

  
          <button className="border border-gray-400 py-2 rounded-lg font-semibold">
            🤍 Add Favorites
          </button>
          ...
          {allTables.map((table, i) => (
            <div key={i} className="bg-[#e8c9b7] p-4 rounded-xl shadow-sm">
              ...
              <button className="mt-3 w-full bg-[#d3b4a4] text-white py-2 rounded-lg">
                + Request To Join
              </button>
            </div>
          ))}
